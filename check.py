    ...
    def evaluate_dataset(self, csv_path, img_dir, output_path=None, sample_count=None):
        ...
        for idx, row in tqdm(df.iterrows(), total=len(df), desc="评估数据集"):
            try:
                ...
                if CONFIG["DEBUG"]:
                    print(f"图像 {row['filename']}: 预测={pred_value}, 真实={true_value}, 正确={correct}")
        ...
        print(f"\n最终准确率: {accuracy:.2%} (容忍度: ±0.5)")
        
        # 可视化结果
        plt.figure(figsize=(10, 6))
        plt.scatter(results_df['true_value'], results_df['pred_value'], alpha=0.6)
        plt.plot([0, 100000], [0, 100000], 'r--')  # 理想线
        plt.xlabel('real')
        plt.ylabel('predict')
        plt.title(f'预测值 vs 真实值 (准确率: {accuracy:.2%})')
        plt.grid(True)
        plt.savefig(f"{CONFIG['OUTPUT_DIR']}/prediction_plot.png")
        plt.show()
